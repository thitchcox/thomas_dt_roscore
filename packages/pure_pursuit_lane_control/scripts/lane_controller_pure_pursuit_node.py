...
class lane_controller_pure_pursuit(object):
    ...
    def __init__(self):
        ...
        # Parameterize the bounding white and yellow lines
        self.white_line_params = {}
        self.yellow_line_params = {}
        ...
        self.sub_filtered_seg_list = rospy.Subscriber("~filtered_segment_list", SegmentList, self.updateControlInputs, queue_size=1)
    ...
    def setGains(self):
        ...
        ##### Set values for pure pursuit
        # The width of the lane [m]
        self.lane_width = self.setupParameter("~lane_width", 0.23)
    
        # d is the lookforward distance along the tangent line [m]
        self.d_lookahead = self.setupParameter("~d_lookahead", 0.3)

        # A factor to increase the turn rate
        self.omega_factor = self.setupParameter("~omega_factor", 1)
        #####
    ...
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    def updateControlInputs(self, filtered_segment_list_msg):

        # Parse the parameters
        msg = filtered_segment_list_msg
        white_seg_endpoints = []
        yellow_seg_endpoints = []

        # Set up the returns
        currentMillis = int(round(time.time() * 1000))
        car_control_msg = Twist2DStamped()

        # Pull out the endpoints
        for segment in msg.segments:
            if segment.color == 0:
                white_seg_endpoints.append([[segment.points[0].x], [segment.points[0].y]])
                white_seg_endpoints.append([[segment.points[1].x], [segment.points[1].y]])
            elif segment.color == 1:
                yellow_seg_endpoints.append([[segment.points[0].x], [segment.points[0].y]])
                yellow_seg_endpoints.append([[segment.points[1].x], [segment.points[1].y]])
            else:
                print("Unrecognized segment color")

        # Fit a line to each set of segment endpoints
        if white_seg_endpoints:
            wep = np.array(white_seg_endpoints)
            coefs = np.polynomial.polynomial.polyfit(np.ndarray.flatten(wep[:, 0]),
                np.ndarray.flatten(wep[:,1]), 1)
            # Assign the white line coefficients.  It's [b, m].
            self.white_line_params["m"] = coefs[1]
            self.white_line_params["b"] = coefs[0]
        if yellow_seg_endpoints:
            yep = np.array(yellow_seg_endpoints)
            coefs = np.polynomial.polynomial.polyfit(np.ndarray.flatten(yep[:,0]), 
                np.ndarray.flatten(yep[:,1]), 1)
            # Assign the yellow line coefficients.
            self.yellow_line_params["m"] = coefs[1]
            self.yellow_line_params["b"] = coefs[0]

        # Need to compute L * sin(alpha) for the pure pursuit control law. 
        # There are 4 cases to consider in terms of what we see in the image. 
        # CASE 1: No line segments visible
        if (not white_seg_endpoints) and (not yellow_seg_endpoints):
            # Drive straight
            car_control_msg.omega = 0.
            car_control_msg.v = self.v_bar
            self.publishCmd(car_control_msg)
            self.last_ms = currentMillis
            return
            
        # CASE 2: Only white lines are visible
        elif white_seg_endpoints and not yellow_seg_endpoints:
            # Need to offset the detected line toward the centre when computing L * sin(alpha)
            slope_vec = np.array([1, self.white_line_params["m"]])
            slope_vec_2 = np.array([self.white_line_params["m"], -1])
            r_rp_b = (slope_vec / np.linalg.norm(slope_vec)) * self.d_lookahead
            r_cr_b = (slope_vec_2 / np.linalg.norm(slope_vec_2)) * self.lane_width
            # Right turn
            if self.white_line_params["m"] >= 0:
                r_cp_b = r_rp_b - r_cr_b
            # Left turn
            else:
                r_cp_b = -r_rp_b - r_cr_b
            # print("White mps only")
            
        # CASE 3: Only yellow lines are visible
        elif yellow_seg_endpoints and not white_seg_endpoints:
            # Need to offset the detected line toward the centre when computing L * sin(alpha)
            slope_vec = np.array([1, self.yellow_line_params["m"]])
            slope_vec_2 = np.array([self.yellow_line_params["m"], -1])
            r_lp_b = (slope_vec / np.linalg.norm(slope_vec)) * self.d_lookahead
            r_cl_b = (slope_vec_2 / np.linalg.norm(slope_vec_2)) * self.lane_width
            # Right turn
            if self.yellow_line_params["m"] >= 0:
                r_cp_b = r_lp_b + r_cl_b
            # Left turn
            else:
                r_cp_b = -r_lp_b + r_cl_b
            # print("Yellow mps only")

        # CASE 4: Both yellow and white lines are visible
        elif white_seg_endpoints and yellow_seg_endpoints:
            # Average the slope for the two lines
            m_avg = (self.white_line_params["m"] + self.yellow_line_params["m"]) / 2
            # Compute L * sin(alpha) using turing parameter 'd' (set locally)
            slope_vec = np.array([1, m_avg])
            r_cp_b = (slope_vec / np.linalg.norm(slope_vec)) * self.d_lookahead

        # Get L * sin(alpha)
        l_sin_alpha = r_cp_b[0]

        # Compute distance L.  Its the norm of the distance to the follow point.
        L = np.linalg.norm(r_cp_b)

        # Keep velocity constant (for now...)
        v = self.v_bar

        # Use l_sin_alpha to compute the control law.  
        omega = (2 * v * l_sin_alpha / L**2) * self.omega_factor  
...
if __name__ == "__main__":
    ...
    rospy.init_node("lane_controller_pure_pursuit_node", anonymous=False)  # adapted to sonjas default file

    lane_control_node = lane_controller_pure_pursuit()
    rospy.spin()
